...
//     // Guard: avoid overlapping recognition calls only
//     if (recognizing) {
//       console.log('⚠️ Already recognizing, skipping this capture');
//       return;
//     }
...
//   const handleFaceRecognition = async (imageFile) => {
//     if (recognizing) {
//       console.log('⚠️ Recognition already in progress, skipping');
//       return;
//     }
...
//         if (userId) {
//           recognizedUser = { userId, userName };
//           setLastRecognizedUser(recognizedUser);
//           toast.success(`✅ Face Recognized: ${userName}`, { autoClose: 2000, position: 'top-center' });
//           console.log(`👤 Recognized user: ${userName} (${userId})`);
//         } else {
//           console.log('⚠️ Face detected but no user ID found');
//           toast.warning('⚠️ Face detected but user ID not found', {
...
//         console.log('❌ Face not recognized');
//         setLastRecognizedUser(null);
//         // Frontend message when user does not exist / not recognized
//         toast.error('❌ Face not recognized. User does not exist in the system.', {
//           autoClose: 3000,
//           position: 'top-center'
//         });
//       }
...
//       });
//     } finally {
//       setRecognizing(false); // free recognition for next capture
//       console.log('🏁 Face recognition complete');
//     }

//     // For every successful recognition, always call journey API
//     if (recognizedUser) {
//       await processJourney(recognizedUser.userId);
//     }
//   };

//   const processJourney = async (userId) => {
//     if (!selectedStation) {
//       toast.error('Please select a station first', { position: 'top-center', autoClose: 3000 });
//       console.log('⚠️ No station selected');
//       return;
//     }

//     // NO cooldown, NO processing guard: every recognition triggers an API call
//     console.log(`🚀 Processing ${operationType} journey for user ${userId} at station ${selectedStation}`);
...
//       // Extract fare from nested path: journey_details.fare_details.total_fare
//       let fareAmount = null;
//       if (result.journey_details?.fare_details) {
//         fareAmount =
//           result.journey_details.fare_details.total_fare ||
//           result.journey_details.fare_details.totalfare;
//       }
...
//         } else {
//           const displayFare =
//             fareAmount !== null && fareAmount !== undefined
//               ? `₹${Number(fareAmount).toFixed(2)}`
//               : 'Calculated';
...
//         // Specific UI when backend says user does not exist
//         if (
...
//           });
//         } else if (
//           errorMsg.toLowerCase().includes('ongoing journey') ||
//           errorMsg.toLowerCase().includes('active journey')
//         ) {
//           toast.error(
//             `⚠️ ${operationType.toUpperCase()} DENIED\nYou have an active journey.\nPlease exit first.`,
//             {
//               autoClose: 5000,
//               position: 'top-center'
//             }
...
//           });
//         } else if (
//           errorMsg.toLowerCase().includes('no fare found') ||
//           errorMsg.toLowerCase().includes('same station') ||
//           errorMsg.toLowerCase().includes('no journey ongoing')
//         ) {
//           toast.error(`🚫 ${operationType.toUpperCase()} DENIED\n${errorMsg}`, {
...
//       console.log('❌ Journey error:', errorMsg);
//       toast.error(`❌ Operation Failed\n${errorMsg}`, { autoClose: 3000, position: 'top-center' });
...
//                 backgroundColor: '#eff6ff',
//                 borderRadius: '8px',
//                 border: '1px solid #3b82f6'  // ✅ fixed quotes
//               }}
//             >
//               <h4 style={{ margin: '0 0 12px 0', color: '#1e40af' }}>📝 Instructions:</h4>
//               <ol style={{ margin: '0', paddingLeft: '20px', color: '#1e3a8a' }}>
...
//                         }}
//                       >
//                         💡 Tip: Complete your ongoing journey at an exit gate before starting a new
//                         one.
//                       </p>
//                     )}
//                   </div>
...
//                   {stations.find((s) => s.station_code === selectedStation)?.station_name ||
//                     selectedStation}
//                 </p>
//                 <p style={{ margin: '8px 0', fontSize: '15px' }}>
//                   <strong>Gate:</strong> {lastResult.gate_open ? '🟢 OPEN' : '🔴 CLOSED'}
...
//                         <strong>Route:</strong>{' '}
//                         {lastResult.journey_details.fare_details.from_station} →{' '}
//                         {lastResult.journey_details.fare_details.to_station}
//                       </p>
//                     )}
...
//                         {lastResult.journey_details.fare_details.distance_km} km
//                       </p>
//                     )}
...
//                         {Number(lastResult.journey_details.fare_details.base_fare).toFixed(2)}
//                       </p>
//                     )}
...
//                         {Number(lastResult.journey_details.fare_details.service_charge).toFixed(2)}
//                       </p>
//                     )}
...
//                         {Number(lastResult.journey_details.fare_details.penalty_fare).toFixed(2)}
//                       </p>
//                     )}
...
//             >
//               <p style={{ fontSize: '16px', marginBottom: '8px' }}>👤 No operations performed yet</p>
//               <p style={{ fontSize: '14px' }}>Start camera to begin continuous scanning</p>
//             </div>
//           )}
//         </Card>
//       </div>
...
const StationOperations = () => {
  ...
  // Timer refs
  const countdownTimerRef = useRef(null);
  const cycleTimeoutRef = useRef(null); // replaces captureIntervalRef

  // 🔑 Ref to avoid stale isCameraActive inside setTimeout
    ...
    try {
      ...
      setTimeout(() => {
        if (videoRef.current) {
          ...
          videoRef.current.play();
          console.log('🔄 Starting continuous capture cycle');
          // start the first countdown+capture cycle
          startCycle();
        }
      }, 100);
  ...
  // Single cycle: countdown 5s -> capture+recognize -> wait 5s -> next cycle
  const startCycle = () => {
    console.log('⏳ Starting countdown from 5');
    setCountdown(5);
    let count = 5;
    ...
    countdownTimerRef.current = setInterval(async () => {
      ...
      if (count <= 0) {
        ...
        // Wait for capture + recognition + journey to finish
        await captureAndRecognize();

        // Schedule next cycle 5 seconds after capture+recognition finishes
        cycleTimeoutRef.current = setTimeout(() => {
          // Only continue if camera is still active (latest value via ref)
          if (isCameraActiveRef.current) {
            console.log('⏰ 5-second delay over - starting next cycle');
            startCycle();
          } else {
            console.log('⏹️ Camera is not active, stopping cycles');
          }
        }, 5000);
      }
    }, 1000);
  ...
  const stopCamera = () => {
    ...
    setStream(null);
    setIsCameraActive(false);
    isCameraActiveRef.current = false; // keep ref in sync
    setCountdown(null);
    setLastRecognizedUser(null);
    setRecognizing(false);
  };
  ...
  const captureAndRecognize = async () => {
    console.log('captureAndRecognize called, recognizing =', recognizing);
    ...
    // Wrap toBlob in a Promise so startCycle can await this
    return new Promise((resolve) => {
      canvas.toBlob(async (blob) => {
        if (!blob) {
          console.log('⚠️ Failed to create image blob');
          resolve();
          return;
        }
        const file = new File([blob], `capture-${Date.now()}.jpg`, { type: 'image/jpeg' });
    ...
    try {
      ...
      if (response.recognized) {
        ...
        if (userId) {
          recognizedUser = { userId, userName };
          setLastRecognizedUser(recognizedUser);
          toast.success(`✅ Face Recognized: ${userName}`, {
            autoClose: 2000,
            position: 'top-center'
          });
          console.log(`👤 Recognized user: ${userName} (${userId})`);
      ...
      setRecognizing(false);
      console.log('🏁 Face recognition complete, recognizing reset to false');
    }

    // Always call journey API on successful recognition
    if (recognizedUser) {
      await processJourney(recognizedUser.userId);
    }
  };
  ...
  console.log('❌ Operation denied:', result.message);
  let errorMsg = result.message || 'Operation denied';

  // Frontend override:
  // If entry operation failed with "System error", treat it as ongoing journey
  if (
    operationType === 'entry' &&
    errorMsg.toLowerCase().includes('system error')
  ) {
    errorMsg = 'User has ongoing journey. Must exit first.';
    // Also update result.message so UI card shows the same text
  ...
  if (
    ...
    errorMsg.toLowerCase().includes('ongoing journey') ||
    errorMsg.toLowerCase().includes('must exit first')
  ) {
    // Show exactly the (possibly overridden) message
